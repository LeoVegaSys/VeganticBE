...
class SQLAgent:
    ...
    def parse_question(self, state: dict) -> dict:
        ...
        print(f"parse_question :: state : {state}")
        question = state['question']
        schema = self.db_manager.get_schema(state['uuid'])
        # schema = _get_schema(state['uuid'])
...
'''),
        ...
        response = self.llm_manager.invoke(prompt, step_name="parse_question", schema=schema, question=question)
        parsed_response = output_parser.parse(response)
        return {"parsed_question": parsed_response}

    def get_unique_nouns(self, state: dict) -> dict:
        """Find unique nouns in relevant tables and columns."""
        print(f"get_unique_nouns :: state : {state}")
        ...
        for table_info in parsed_question['relevant_tables']:
            ...
            if noun_columns:
                ...
                # results = _execute_query(state['uuid'], query)
                for row in results:
    ...
    def generate_sql(self, state: dict) -> dict:
        ...
        print(f"generate_sql :: state : {state}")
        question = state['question']
        parsed_question = state['parsed_question']
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", '''
You are an AI assistant that generates MySQL queries based on user questions, database schema, and unique nouns found in the relevant tables. Generate a valid SQL query to answer the user's question.
...
Generate SQL query string'''),
        ...
        response = self.llm_manager.invoke(prompt, step_name="generate_sql", schema=schema, question=question, parsed_question=parsed_question, unique_nouns=unique_nouns)
    ...
    def validate_and_fix_sql(self, state: dict) -> dict:
        ...
        print(f"validate_and_fix_sql :: state : {state}")
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", '''
You are an AI assistant that validates and fixes MySQL queries. Your task is to:
...
3. If there are any issues, fix them and provide the corrected MySQL query.
...
'''),
        ...
        output_parser = JsonOutputParser()
        response = self.llm_manager.invoke(prompt, step_name="validate_and_fix_sql", schema=schema, sql_query=sql_query)
    ...
    def execute_sql(self, state: dict) -> dict:
        ...
        print(f"execute_sql :: state : {state}")
        ...
        try:
            ...
            # results = _execute_query(uuid, query)
    ...
    def format_results(self, state: dict) -> dict:
        ...
        print(f"format_results :: state : {state}")
        question = state['question']
    ...
    def choose_visualization(self, state: dict) -> dict:
        ...
        print(f"choose_visualization :: state : {state}")
        question = state['question']
        results = state['results']
        sql_query = state['sql_query']
...
Recommend a visualization:'''),
        ...
        response = self.llm_manager.invoke(prompt, step_name="choose_visualization", question=question, sql_query=sql_query, results=results)
